    ...
    return {
        ...
        template: '<div class="panel-group" id="{{panelId}}">\
                       <div class="panel panel-default" ng-repeat-start="item in ngModel">\
                           <div class="panel-heading">\
                               <h4 class="panel-title">\
<a ng-click="toggleCollapsedStates($index)" href="">{{item.id}}- {{item.taskName}}</a>\
                               </h4>\
                           </div>\
<div id="{{panelBaseId}}-{{$index}}" data-parent="#{{panelId}}" class="panel-collapse collapse">\
                               <div class="panel-body">{{item.taskDesc}} <br> <strong>Assigned to :</strong> {{item.assignedTo}} <br> <strong> Due Date:</strong>{{item.dueDate}} <br> <strong>Status:</strong>{{item.status}}</div>\
                           </div>\
                       </div>\
                       <div ng-repeat-end></div>\
                   </div>',
